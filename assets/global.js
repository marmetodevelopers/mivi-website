...
document.addEventListener("DOMContentLoaded", function () {
  // footer for mobile
  document
    .querySelectorAll(".footer__blocks-wrapper .footer__trigger-accordion")
    .forEach((each) => {
  ...
  document.getElementById("myButton")?.addEventListener("click", function (e) {
    ...
    if (len) {
      ...
      document.body.classList.remove("add-to-cart-sticky-active");
      ...
      const stickyContainer = document.querySelector(
        ".mobile-add-cart-sticky-container"
      );
      stickyContainer.style.opacity = "0";
      stickyContainer.style.position = "relative";
      stickyContainer.style.zIndex = "-1";

      let assignEvent = setInterval(() => {
        if (document.querySelector(".cd-cart-close-button")) {
          document
            .querySelector(".cd-cart-close-button")
            .addEventListener("click", (e) => {
              // Find the .mobile-add-cart-sticky-container element
              const stickyContainer = document.querySelector(
                ".mobile-add-cart-sticky-container"
              );
              ...
              stickyContainer.style.zIndex = "99999"; // Set zIndex to an empty string to remove it
            });
          clearInterval(assignEvent);
        }
      });
    } else {
      let submitButton = document.querySelector(".product-form__submit");
      if (window.innerWidth < 749 && submitButton) {
        submitButton.click();
      }
    }
  });
...
if (targetNode) {
  const textElements = document.querySelectorAll(
    ".faqs-container .questions_wrapper"
  );
  ...
  const callback = function (mutationsList, observer) {
    for (const mutation of mutationsList) {
      if (mutation.type === "attributes") {
        ...
        document
          .querySelector(
            `.faqs-container .questions_wrapper[slide-index="${activeSplideIndex}"]`
          )
